...
class SettingsView(QWidget):
    ...
    def _search_settings(self, query):
        ...
        keywords = (
            ...
            ("processing compute cpu gpu cuda queue concurrent", 2),
    ...
    def save(self):
        self.settings = replace(self.settings, output_directory=self.output_path.text(), model_directory=self.model_path.text(), theme=self.theme.currentText(), update_checks=self.update_checks.isChecked(), reduced_motion=self.reduced_motion.isChecked(), cache_limit_gb=self.cache_limit.value(), compute_device=self.compute_device.currentText(), queue_concurrency=self.queue_concurrency.value(), subtitle_font_size=self.subtitle_size.value(), subtitle_line_length=self.line_length.value(), log_level=self.log_level.currentText())
        try:
